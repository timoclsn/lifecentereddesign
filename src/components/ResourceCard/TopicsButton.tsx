...
export const TopicsButton = ({ children, topicName }: Props) => {
  ...
  const handleClick = () => {
    ...
    handleValueChange('topic', topicName);
  };

  if (!inContext) {
    return (
      <Link
        href={`/resources?topic=${topicName.toLowerCase()}#resources`}
        prefetch={false}
